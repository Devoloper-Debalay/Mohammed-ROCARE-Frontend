...
export function VendorNotificationsPage() {
  ...
  const markRead = async (id: string) => {
    await vendorApi.patch(`/vendor/notifications/${id}/read`);
    load();
  };

  const markAllRead = async () => {
    await vendorApi.patch("/vendor/notifications/read-all");
    load();
  };
  ...
  return (
    <div>
      <PageHeader
        eyebrow="Updates"
        title="Notifications"
        description="Lead, wallet and account updates."
        action={
          unreadCount > 0 ? (
            <Button accent="orange" variant="secondary" onClick={markAllRead}>
              Mark all read
            </Button>
          ) : undefined
        }
      ...
      {loading ? (
        ...
        <div className="flex flex-col gap-2">
          {items.map((n) => (
            <Card key={n.id} className={`flex items-start justify-between gap-4 p-4 ${!n.isRead ? "border-orange/30" : ""}`}>
              <div>
              ...
              {!n.isRead && (
                <button onClick={() => markRead(n.id)} className="shrink-0 text-xs font-semibold text-orange-deep">
                  Mark read
                </button>
              )}
            </Card>
