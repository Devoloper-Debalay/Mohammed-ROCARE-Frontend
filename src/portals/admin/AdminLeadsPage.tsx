...
const DEFAULT_LEADS: Lead[] = [
  { id: "LEAD-KOL-101", customerName: "Dr. Anirban Sen", serviceType: "Kent Grand Plus RO Membrane Replacement", status: "PENDING_START_VERIFICATION", area: "Sector 1, Salt Lake", technicianName: "Subhashish Roy", charge: 60, createdAt: new Date().toISOString() },
  { id: "LEAD-KOL-102", customerName: "Smt. Priyanka Roy", serviceType: "Voltas 1.5 Ton Inverter AC Gas Refill", status: "ONGOING", area: "Behala Tram Depot", technicianName: "Tanmoy Mukherjee", charge: 50, createdAt: new Date(Date.now() - 3600000).toISOString() },
  { id: "LEAD-KOL-103", customerName: "Rajiv Poddar", serviceType: "AO Smith Geyser Heating Coil Fix", status: "PENDING_DENIAL_VERIFICATION", area: "Dunlop, Baranagar", technicianName: "Bikash Sarkar", charge: 45, createdAt: new Date(Date.now() - 7200000).toISOString() },
  { id: "LEAD-KOL-104", customerName: "Amitabh Bhattacharya", serviceType: "Aquaguard Annual Maintenance Service", status: "COMPLETED", area: "Howrah AC Market", technicianName: "Subhashish Roy", charge: 60, createdAt: new Date(Date.now() - 14400000).toISOString() },
];

const DEFAULT_START_PROOFS: Proof[] = [
  { id: "PRF-STR-01", leadId: "LEAD-KOL-101", customerName: "Dr. Anirban Sen", technicianName: "Subhashish Roy", latitude: 22.5850, longitude: 88.4180, submittedAt: new Date().toISOString(), imageUrl: "https://images.unsplash.com/photo-1584622650111-993a426fbf0a?w=500&auto=format&fit=crop&q=60" },
];

const DEFAULT_DENIAL_PROOFS: Proof[] = [
  { id: "PRF-DEN-01", leadId: "LEAD-KOL-103", customerName: "Rajiv Poddar", technicianName: "Bikash Sarkar", reason: "Customer not at home after 3 phone attempts and door bell ringing.", latitude: 22.6520, longitude: 88.3760, submittedAt: new Date(Date.now() - 7200000).toISOString() },
];
              ...
              {startProofs.map((p) => (
                <div key={p.id} className="rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 p-4 space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="font-mono font-bold text-xs text-blue-600 dark:text-blue-400">#{p.leadId}</span>
                  ...
                  <div>
                    <p className="font-bold text-sm text-gray-900 dark:text-white">Customer: {p.customerName || "Customer"}</p>
                    <p className="text-xs text-gray-600 dark:text-gray-400">Technician: <strong>{p.technicianName || "Subhashish Roy"}</strong></p>
                  ...
                  {p.imageUrl && (
                    <div className="relative rounded-xl overflow-hidden h-36 border border-gray-300 dark:border-gray-600">
                      <img src={p.imageUrl} alt="Geotag proof" className="h-full w-full object-cover" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {denialProofs.map((p) => (
              <div key={p.id} className="rounded-xl border border-rose-200 dark:border-rose-900 bg-rose-50/40 dark:bg-rose-950/20 p-4 space-y-3">
                <div className="flex items-center justify-between">
                  <span className="font-mono font-bold text-xs text-rose-600 dark:text-rose-400">#{p.leadId}</span>
                ...
                <div>
                  <p className="font-bold text-sm text-gray-900 dark:text-white">Customer: {p.customerName || "Customer"}</p>
                  <p className="text-xs text-gray-600 dark:text-gray-400">Technician: <strong>{p.technicianName || "Bikash Sarkar"}</strong></p>
